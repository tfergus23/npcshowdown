...
inline constexpr float statStageMultiplier(int stage){
    int index = stage + 6;
    return statMultipliers[index];
}
inline constexpr float accuracyStageMultiplier(int accuracyStage){
    int index = accuracyStage + 6;
    return accuracyMultipliers[index];
}
inline constexpr float critChanceFromStage(int stage){
